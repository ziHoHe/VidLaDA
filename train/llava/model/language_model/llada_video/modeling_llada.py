...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    def generate_with_embeds(self, inputs_embeds, steps=128, gen_length=128, block_length=128, temperature=0.,
        ...
        '''
        generation_context_ids = kwargs.get("generation_context_ids", None)
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            if generation_context_ids is not None:
                generation_context_ids = generation_context_ids.to(device=inputs_embeds.device)
                assert generation_context_ids.shape[1] == gen_length
                generation_context_embeds = self.model.embed_tokens(generation_context_ids)
            ...
            if generation_context_ids is not None:
                start_gen = inputs_embeds.shape[1]
                end_gen = start_gen + gen_length
                x_embeds[:, start_gen:end_gen] = generation_context_embeds
            ...
            if generation_context_ids is not None:
                start_gen = inputs_embeds.shape[1]
                end_gen = start_gen + gen_length
                x[:, start_gen:end_gen] = generation_context_ids
            ...
            for num_block in range(num_blocks):
                ...
                for i in range(steps):
                    current_block_embeds = x_embeds[:, block_start:block_end]
                    current_block_mask_index = torch.all(torch.abs(current_block_embeds - masked_embed) < 1e-5, dim=2)

                    if found_stop_seq:
                        pre_stop_masks = torch.all(torch.abs(x_embeds[:, inputs_embeds.shape[1]:stop_position] - masked_embed) < 1e-5, dim=2)
                    ...
                    if cfg_scale > 0.:
                        ...
                        block_hidden_states = outputs[0][:, block_start:block_end, :]
                        
                        logits = self.lm_head(block_hidden_states).float()
                        
                        logits, un_logits = torch.chunk(logits, 2, dim=0)
                        logits = un_logits + (cfg_scale + 1) * (logits - un_logits)
                    else:
                        import os
                        if os.getenv("OUTPUT_ATTENTIONS", "FALSE") == "TRUE":
                            save_attentions_func = kwargs.get("save_attentions_func", None)
                            assert self.config._attn_implementation == "sdpa" or self.config._attn_implementation == "eager"
                            outputs = self.model(inputs_embeds=x_embeds, output_attentions=True, output_hidden_states=True)
                            attentions = outputs.attentions
                            output_hidden_states = outputs.hidden_states
                            cur_step = i * num_block + i
                            assert save_attentions_func is not None
                            save_attentions_func(attentions, output_hidden_states, gen_length, block_length, steps * num_blocks, cur_step)
                    ...
                    for token_id in [126081, 126080, 126346, 126347]:
                        logits[:, :, token_id] = torch.where(current_block_mask_index, -float('inf'), logits[:, :, token_id])
                    
                    logits_with_noise = self.add_gumbel_noise(logits, temperature=temperature)
                    x0_block = torch.argmax(logits_with_noise, dim=-1)
                    
                    if remasking == 'low_confidence':
                        p_block = F.softmax(logits.to(torch.float64), dim=-1)
                        x0_p_block = torch.squeeze(
                            torch.gather(p_block, dim=-1, index=torch.unsqueeze(x0_block, -1)), -1)
                    elif remasking == 'random':
                        x0_p_block = torch.rand((x0_block.shape[0], x0_block.shape[1]), device=x0_block.device)
                    else:
                        raise NotImplementedError(remasking)

                    if found_stop_seq and stop_position < block_end:
                        rel_stop_pos = max(0, stop_position - block_start)
                        x0_p_block[:, rel_stop_pos:] = -np.inf
                    
                    if suffix_len > 0:
                        suffix_start_global = total_length - suffix_len
                        if suffix_start_global < block_end:
                            rel_suffix_start = max(0, suffix_start_global - block_start)
                            x0_p_block[:, rel_suffix_start:] = -np.inf

                    x0_embeds_block = self.model.embed_tokens(x0_block)

                    confidence_block = torch.where(current_block_mask_index, x0_p_block, -np.inf)
                    

                    transfer_index_block = torch.zeros_like(x0_block, dtype=torch.bool, device=x0_block.device)
                    for j in range(confidence_block.shape[0]):
                        k_val = num_transfer_tokens[j, i]
                        if k_val > 0:
                            _, select_index = torch.topk(confidence_block[j], k=k_val)
                            transfer_index_block[j, select_index] = True
                    
                    target_embeds_slice = x_embeds[:, block_start:block_end]
                    target_ids_slice = x[:, block_start:block_end]
                    
                    target_embeds_slice[transfer_index_block] = x0_embeds_block[transfer_index_block]
                    target_ids_slice[transfer_index_block] = x0_block[transfer_index_block]
    ...
    def forward(
        ...
        force_mask: Optional[torch.LongTensor] = None, # [True, False, True, ...], True: mask
        ...
        def forward_process_embeds(input_embeds, labels, eps=1e-3):
            ...
            if force_mask is not None:
                p_mask = torch.where(force_mask, torch.ones_like(p_mask,), p_mask)
