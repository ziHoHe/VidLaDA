...
def build_vision_tower(vision_tower_cfg, **kwargs):
    ...
    if VISION_TOWER_CACHE_DIR := os.environ.get("VISION_TOWER_CACHE_DIR", ""):
        vision_tower_path = os.path.join(VISION_TOWER_CACHE_DIR, vision_tower)
    ...
    if vision_tower.startswith("openai") or vision_tower.startswith("laion") or "ShareGPT4V" in vision_tower: # is_absolute_path_exists or 
        ...
        return SigLipVisionTower(vision_tower_path, vision_tower_cfg=vision_tower_cfg, **kwargs)
    elif vision_tower.startswith("hf:"):
        return HFVisionTower(vision_tower_path, args=vision_tower_cfg, **kwargs)
    elif vision_tower in ["imagebind_huge"]:
        return ImageBindWrapper(vision_tower_path, args=vision_tower_cfg, **kwargs)
