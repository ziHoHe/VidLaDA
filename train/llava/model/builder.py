...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
            ...
            elif "llada" in model_name.lower() or "vidlada" in model_name.lower():
                
                if "llada_video" in model_path.lower() or "llada-video" in model_path.lower() or "vidlada" in model_name.lower():
                    from llava.model.language_model.llava_llada_video import LlavaLLaDAVideoConfigonfig
                    CONFIG_CLS = LlavaLLaDAVideoConfigonfig
                    MODEL_CLS = LlavaLLaDAVideoModelLM
                else:
                    from llava.model.language_model.llava_llada import LlavaLLaDAConfig
                    CONFIG_CLS = LlavaLLaDAConfig
                    MODEL_CLS = LlavaLLaDAModelLM

                tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
                if customized_config is None:
                    llada_cfg = CONFIG_CLS.from_pretrained(model_path)
                else:
                ...
                model = MODEL_CLS.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llada_cfg, **kwargs)
