...
class ModelArguments:
    ...
    train_pad_token: Optional[bool] = field(default=False)
    force_pad_to_max_length: Optional[bool] = field(default=False)
...
class DataArguments:
    ...
    image_folder_VisualWebInstruct: Optional[str] = field(default=None)
    ...
    video_folder_ShareGPTVideo: Optional[str] = field(default=None)
    video_folder_video_r1: Optional[str] = field(default=None)
    video_folder_finevideo: Optional[str] = field(default=None)
    video_folder_MAmmoTHVL: Optional[str] = field(default=None)
    video_fps: Optional[int] = field(default=1)
    frames_upbound: Optional[int] = field(default=0)
    add_time_instruction: Optional[bool] = field(default=False)
    force_sample: Optional[bool] = field(default=False)
    
    think_mode: Optional[str] = field(default=None)
    mask_think_ratio: Optional[float] = field(default=None)
    random_pad_stop_think: Optional[int] = field(default=None)
    add_answer_token: bool = field(default=True)
    force_mc_training: Optional[bool] = field(default=None)
...
def preprocess_llada_cot(
    sources,
    tokenizer: transformers.PreTrainedTokenizer,
    has_image: bool = False,
    max_len=2048,
    system_message: str = "You are a helpful language and vision assistant. You are able to understand the visual content that the user provides, and assist the user with a variety of tasks using natural language.",
    add_answer_token: bool = False
) -> Dict:
    roles = {"human": "user", "gpt": "assistant"}
    ...
    end_of_think_id = tokenizer.convert_tokens_to_ids("</think>")
    stop_think = tokenizer.convert_tokens_to_ids("<stop_think>")
    if add_answer_token:
        answer_id = tokenizer.convert_tokens_to_ids("<answer>")
    end_of_answer_id = tokenizer.convert_tokens_to_ids("</answer>")

    unmask_tokens = ["<|startoftext|>", "<|start_header_id|>", "<|end_header_id|>", "<|eot_id|>", "\n\n"] + ["<think>", "</think>", "<stop_think>", ] + ["<answer>", "</answer>"] if add_answer_token else []
    unmask_tokens_idx = [tokenizer.convert_tokens_to_ids(tok) for tok in unmask_tokens]
    # Reset LLaDA chat templates so that it won't include assistant message every time we apply
    chat_template = "{% for message in messages %}{{'<|startoftext|>' + '<|start_header_id|>' + message['role'] + '<|end_header_id|>' + '\n\n' + message['content'] + '<|eot_id|>'}}{% endfor %}{% if add_generation_prompt %}{{ '<|start_header_id|>assistant<|end_header_id|>\n\n' }}{% endif %}"
    tokenizer.chat_template = chat_template
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, **kwargs) -> Dict:
    """
    ...
    if conversation_lib.default_conversation.version == "llava_llada_cot":
        return preprocess_llada_cot(sources, tokenizer, has_image=has_image, **kwargs)
...
class WebDatasetSupervisedDataset(torch.utils.data.IterableDataset):
    ...
    def process_sample(self, sample):
        ...
        try:
            ...
            data_dict = preprocess(sources, self.tokenizer, has_image=has_image, add_answer_token=self.data_args.add_answer_token)
...
class LazySupervisedDataset(Dataset):
            ...
            with open(data_path, "r") as file:
                ...
                for dataset in datasets:
                    ...
                    if "," in sampling_strategy:
                        sampling_strategy, epoch = sampling_strategy.split(",")
                        epoch = int(epoch)
                    assert epoch >= 1
                    ...
                    if sampling_strategy == "first" and sampling_number is not None:
                        ...
                        cur_data_dict = cur_data_dict
                    else:
                        raise NotImplementedError(f"unkown {sampling_strategy}")
                    
                    cur_data_dict = cur_data_dict * epoch

                    rank0_print(f"Loaded {len(cur_data_dict)} samples from {json_path}, train {epoch} epoch.")
                    self.list_data_dict.extend(cur_data_dict)
        else:
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        is_image_path = False
        image_folders = [
            self.data_args.image_folder, self.data_args.image_folder_2, self.data_args.image_folder_VisualWebInstruct, self.data_args.video_folder_video_r1,
        ]
        for image_folder in image_folders:
            if image_folder is None:
                continue
            temp_image_file = os.path.join(image_folder, image_file)
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        think_mode = None
        if self.data_args.training_cot:
            # process cot
            if self.data_args.mask_think_ratio is not None:
                mask_think = random.random() < self.data_args.mask_think_ratio
                think_mode = "think" if not mask_think else "no_think"
            else:
                think_mode = "think"
            
            # gpt output
            # assert "process" in sources[0]
            if "process" in sources[0]:
                if think_mode == "think":
                    process = sources[0]["process"].strip("<think>").strip("</think>").strip("\n")
                    solution = sources[0]["conversations"][1]["value"].strip("\n").strip("<answer>").strip("</answer>").strip("\n")
                    if self.data_args.add_answer_token:
                        solution = f"<answer>{solution}</answer>"
                    sources[0]["conversations"][1]["value"] = f"<think>{process}</think>" + "\n" + solution
            else:
                think_mode = "no_think"
            
        else:
            think_mode = self.data_args.think_mode
        ...
        if "image" in sources[0]:
            ...
            is_ShareGPTVideo = False
            is_video_path = False
            if "shareVideoGPTV" in video_file:
                video_folder = self.data_args.video_folder_ShareGPTVideo
                video_file = os.path.join(video_folder, os.path.basename(video_file))
                is_ShareGPTVideo = True
                is_video_path = True
            elif "ShareGPTVideo" in video_file:
                video_folder = self.data_args.video_folder_MAmmoTHVL
                video_file = os.path.join(video_folder, video_file)
                is_ShareGPTVideo = True
                is_video_path = True
            else:
                video_folders = [self.data_args.video_folder, self.data_args.video_folder_video_r1, self.data_args.video_folder_finevideo, self.data_args.video_folder_MAmmoTHVL]
                for video_folder in video_folders:
                    if video_folder is None:
                        continue
                    temp_video_file = os.path.join(video_folder, video_file)
            ...
            if not is_video_path or not os.path.exists(video_file):
        ...
        if think_mode is not None:
            assert sources[0][0]["from"] == "human" 
            if think_mode == "no_think":
                sources[0][0]["value"] += "/no_think"
            elif think_mode == "think":
                sources[0][0]["value"] += "/think"
            else:
                raise NotImplementedError()
        ...
        if self.data_args.force_mc_training is not None and self.data_args.force_mc_training:
            assert sources[0][1]["from"] == "gpt"
            if (response_len := len(self.tokenizer.encode(sources[0][1]["value"]))) == 1:
                data_dict["force_mask"] = True
            else:
                data_dict["force_mask"] = False
        ...
        elif conversation_lib.default_conversation.version in ["llava_llada", "llava_llada_cot"]:
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "is_llada" in instances[0] and instances[0]["is_llada"]:
            ...
            if "force_mask" in instances[0]:
                batch["force_mask"] = torch.tensor([instance["force_mask"] for instance in instances], dtype=torch.bool, device=input_ids.device)
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.train_pad_token:
        overwrite_config["train_pad_token"] = model_args.train_pad_token
    ...
    if model_args.model_class_name is not None:
        ...
        elif "llada" in model_args.model_name_or_path.lower() or "vidlada" in model_args.model_name_or_path.lower():
            if model_args.force_pad_to_max_length is not None:
                setattr(customized_kwargs["config"], "force_pad_to_max_length", model_args.force_pad_to_max_length)
            
            model_path_lower = model_args.model_name_or_path.lower()
            if "llada-video" in model_path_lower or "llada_video" in model_path_lower or "vidlada" in model_path_lower:
                MODEL_CLS = LlavaLLaDAVideoModelLM
            else:
                MODEL_CLS = LlavaLLaDAModelLM
...
def train(attn_implementation=None):
    ...
    if "mistral" in model_args.model_name_or_path.lower() or "mixtral" in model_args.model_name_or_path.lower() or "zephyr" in model_args.model_name_or_path.lower():
        ...
        or "vidlada" in model_args.model_name_or_path.lower()
    ...
    if model_args.version == "llava_llada_cot":
        added_tokens = ["<think>", "</think>", "<stop_think>",]
        if data_args.add_answer_token:
            added_tokens += ["<answer>", "</answer>"]
        
        num_new_tokens = tokenizer.add_tokens(added_tokens, special_tokens=True)
        assert num_new_tokens == len(added_tokens)
        model.resize_token_embeddings(len(tokenizer))
    ...
    for n, p in model.named_parameters():
        rank0_print(f"{n}: {p.requires_grad}")
    ...
    data_args.training_cot = training_args.training_cot = "cot" in model_args.version.lower()
    if training_args.training_cot:
        rank0_print(f"Constructing CoT dataset format.")
